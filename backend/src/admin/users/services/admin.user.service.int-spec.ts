    ...
    describe('getAllUsers', () => {
        it('returns only non-admin, non-archived users', async () => {
            await prisma.user.createMany({
                ...
                { email: 'pm@test.com', fullName: 'Project Manager', role: 'PROJECT_MANAGER' },
                { email: 'consultant@test.com', fullName: 'Consultant User', role: 'CONSULTANT' },
                { email: 'archived@test.com', fullName: 'Archived User', role: 'CONSULTANT', status: 'ARCHIVED', deletedAt: new Date() },
                ],
            });
        ...
        it('returns correct activeUsers and suspendedUsers counts', async () => {
            await prisma.user.createMany({
                data: [
                    { email: 'active1@test.com', fullName: 'Active One', role: 'CONSULTANT', status: 'ACTIVE' },
                    { email: 'active2@test.com', fullName: 'Active Two', role: 'PROJECT_MANAGER', status: 'ACTIVE' },
                    { email: 'suspended@test.com', fullName: 'Suspended One', role: 'CONSULTANT', status: 'SUSPENDED' },
                ],
            });
        ...
        it('returns empty array when no non-admin users exist', async () => {
            await prisma.user.create({
                data: { email: 'admin@test.com', fullName: 'Admin Only', role: 'ADMIN' },
            });
