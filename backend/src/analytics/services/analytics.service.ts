...
export class AnalyticsService {
  ...
  async getUtilisationBySkillCategory(): Promise<UtilisationBySkillDto[]> {
    ...
    return Array.from(byCategory.entries()).map(([category, consultants]) => {
      ...
      const utilisedConsultants = Array.from(consultants.values()).filter(
        (c) => c < 100,
      ).length;
      return {
        category,
        totalConsultants,
        utilisedConsultants,
        utilisationPercent: this.toPercent(
          utilisedConsultants,
          totalConsultants,
        ),
      };
  ...
  async getBenchBySkillCategory(): Promise<BenchBySkillDto[]> {
    ...
    return Array.from(byCategory.entries()).map(([category, consultants]) => ({
      category,
      benchCount: Array.from(consultants.values()).filter((c) => c === 100)
        .length,
    }));
  }
  ...
  async getPlacementsBySkillCategory(): Promise<PlacementsBySkillDto[]> {
    ...
    for (const placement of placements) {
      ...
      const categories = new Set(
        placement.project.skills.map((s) => s.skill.category),
      );
      for (const category of categories) {
  ...
  async getCvParsingStats(): Promise<CvParsingStatsDto> {
    ...
    const ruleBasedCount = processedFiles.filter(
      (f) => f.parsingMethod === 'RULE_BASED',
    ).length;
    const aiAssistedCount = processedFiles.filter(
      (f) => f.parsingMethod === 'AI_ASSISTED',
    ).length;
    const successCount = processedFiles.filter(
      (f) => f.extractionStatus === 'REVIEW_REQUIRED',
    ).length;
    const failedCount = processedFiles.filter(
      (f) => f.extractionStatus === 'FAILED',
    ).length;
    ...
    const confidenceScores = processedFiles
      ...
      .map(
        (f) =>
          (f.parsedData as { data?: ParsedCvData })?.data?.confidenceScores
            ?.overall,
      )
  ...
  private async groupConsultantsByCategory(): Promise<
