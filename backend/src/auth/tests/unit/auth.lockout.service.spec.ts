...
interface PrismaUserDelegate {
  update: jest.MockedFunction<(args: Prisma.UserUpdateArgs) => Promise<User | undefined>>;
}
...
describe('LockoutService', () => {
  ...
  describe('recordFailedAttempt()', () => {
    describe('below the lockout threshold', () => {
      ...
      it('does not set status or lockedUntil before the threshold', async () => {
        ...
        const data = mockPrismaUser.update.mock.calls[0]?.[0]?.data;
  ...
  describe('remainingLockoutSeconds()', () => {
    ...
    it('rounds up partial seconds (Math.ceil)', () => {
      const user = makeUser({ lockedUntil: new Date(Date.now() + 1500) });
      const seconds = service.remainingLockoutSeconds(user);
  ...
  // -------------------------------------------------------------------------
  // unlockAccount (admin action)
  // -------------------------------------------------------------------------

  describe('unlockAccount()', () => {
    it('calls Prisma with the correct unlock payload', async () => {
      const unlockedUser = makeUser();
      mockPrismaUser.update.mockResolvedValue(unlockedUser);
    ...
    it('returns the updated user from Prisma', async () => {
      ...
      const result = await service.unlockAccount(user.id);
  ...
  // -------------------------------------------------------------------------
  // lockAccount (admin action)
  // -------------------------------------------------------------------------

  describe('lockAccount()', () => {
    it('calls Prisma with the correct lock payload', async () => {
      mockPrismaUser.update.mockResolvedValue(undefined);
    ...
    it('resolves without returning a value', async () => {
      ...
      await expect(service.lockAccount('user-1')).resolves.toBeUndefined();
    });
  });
});