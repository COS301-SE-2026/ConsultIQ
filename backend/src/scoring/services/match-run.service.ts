...
export class MatchRunService {
    ...
    async executeMatchRun(
        ...
        if (project.status !== 'OPEN' && project.status !== "IN_PROGRESS") {
            throw new BadRequestException(`Match run can only be initialized for open and in-progress projects. Project Status is ${project.status}`);
            ...
            const { activeWeights } = await this.dataIngestion.ingestData({
                ...
                projectId,
                consultant: { consultantId: 'config', skills: [], costToCompany: 0, city: '', province: '' } as any,
                project: projectDto,
            })
