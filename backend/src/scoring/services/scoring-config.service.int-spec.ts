...
describe('ScoringService - Two-Tier Config Integration Test', () => {
    ...
    beforeAll(async () => {
        const moduleRef: TestingModule = await Test.createTestingModule({imports: [ScoringModule],}).compile();
    ...
    function createTestProject(overrides: Partial<Parameters<typeof prisma.project.create>[0]['data']> = {}) {
    ...
    describe('resolveProjectWeights - two-tier fallback chain', () => {
        it('should return project-level overrides when they exist for the project', async () => {
            const project = await createTestProject({ projectName: 'Override Project', allocation: 10 });
        ...
        it('should fall back to firm-wide ConsultancyScoringConfig when no project overrides exist', async () => {
            const project = await createTestProject({ projectName: 'No Override Project' });
            await seedFirmWideConfig();
        ...
        it('should ignore inactive project overrides and fall back to firm-wide config', async () => {
            const project = await createTestProject({ projectName: 'Inactive Override Project' });
            await seedFirmWideConfig();
        ...
        it('should return an empty array when neither overrides nor firm-wide config are seeded', async () => {
            const project = await createTestProject({ projectName: 'Empty Config Project' });
