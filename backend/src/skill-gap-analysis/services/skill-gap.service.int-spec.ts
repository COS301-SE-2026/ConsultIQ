...
describe('SkillGapService - Integration Test', () => {
    ...
    async function createTestProject(
        overrides: Partial<Parameters<typeof prisma.project.create>[0]['data']> = {},
    ) {
    ...
    describe('getProjectSkillGapAnalysis', () => {
        it('should return 100% coverage (COVERED) when enough qualified consultants are available', async () => {
            ...
            const project = await createTestProject({
                teamSize: 2,
                skills: {
                    create: [{ skillId: reactSkill.id, competency: CompetencyLevel.INTERMEDIATE, years: 2 }]
                }
            });

            await createTestConsultant(ConsultantAvailability.AVAILABLE, [{ skillId: reactSkill.id, competencyLevel: CompetencyLevel.INTERMEDIATE }]);
            await createTestConsultant(ConsultantAvailability.AVAILABLE, [{ skillId: reactSkill.id, competencyLevel: CompetencyLevel.EXPERT }]);


            const result = await skillGapService.getProjectSkillGapAnalysis(project.id);
        ...
        it('should accurately calculate AT_RISK severity and ignore unqualified or unavailable consultants', async () => {
            ...
            const project = await createTestProject({
                teamSize: 2,
                skills: {
                    create: [{ skillId: nodeSkill.id, competency: CompetencyLevel.EXPERT, years: 5 }]
                }
            });

            await createTestConsultant(ConsultantAvailability.AVAILABLE, [{ skillId: nodeSkill.id, competencyLevel: CompetencyLevel.EXPERT }]);

            await createTestConsultant(ConsultantAvailability.UNAVAILABLE, [{ skillId: nodeSkill.id, competencyLevel: CompetencyLevel.EXPERT }]);

            await createTestConsultant(ConsultantAvailability.AVAILABLE, [{ skillId: nodeSkill.id, competencyLevel: CompetencyLevel.BEGINNER }]);
    ...
    describe('getPortfolioSkillGapAnalysis', () => {
        it('should aggregate skills across all OPEN/IN_PROGRESS projects and generate correct alerts', async () => {
            ...
            await createTestProject({
                ...
                skills: {
                    create: [
                        { skillId: javaSkill.id, competency: CompetencyLevel.EXPERT, years: 5 },
                        { skillId: angularSkill.id, competency: CompetencyLevel.BEGINNER, years: 1 }
                    ]
                }
            });
            ...
            await createTestProject({
                ...
                teamSize: 5,
                skills: { create: [{ skillId: angularSkill.id, competency: CompetencyLevel.EXPERT, years: 5 }] }
            });


            await createTestConsultant(ConsultantAvailability.AVAILABLE, [{ skillId: javaSkill.id, competencyLevel: CompetencyLevel.EXPERT }]);

            const result = await skillGapService.getPortfolioSkillGapAnalysis();

            expect(result.skills.length).toBe(2);

            const aggregatedJava = result.skills.find(s => s.skillId === javaSkill.id);
            ...
            const aggregatedAngular = result.skills.find(s => s.skillId === angularSkill.id);
