...
export class SkillGapService {
    ...
    async getProjectSkillGapAnalysis(projectId: string) {
        ...
        const skills = await Promise.all(
            project.skills.map(async (projectSkill): Promise<SkillGapItem> => {
                ...
                const coveragePercent = requiredCount > 0
                    ? (availableCount / requiredCount) * 100
                    : 100;
        ...
        const overallCoveragePercent = skills.length > 0
    ...
    async getPortfolioSkillGapAnalysis() {
        ...
        const portfolioSkillsMap = new Map<string, {
            skillName: string;
            requiredCount: number;
        ...
        const aggregatedSkills = await Promise.all(
            Array.from(portfolioSkillsMap.entries()).map(async ([skillId, data]): Promise<SkillGapItem> => {
                const validCompetencies = getValidCompetencies(data.minCompetencyNeeded);
        ...
        const coveredCount = aggregatedSkills.filter(s => s.severity === "COVERED").length;
        const atRiskCount = aggregatedSkills.filter(s => s.severity === "AT_RISK").length;
        const criticalCount = aggregatedSkills.filter(s => s.severity === "CRITICAL").length;
        const overallCoveragePercent = aggregatedSkills.length > 0
            ? aggregatedSkills.reduce((acc, curr) => acc + curr.coveragePercent, 0) / aggregatedSkills.length
            : 100;
